...
void OLED_ClearArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height) {
  ...
  for (j = Y; j < Y + Height; j++) // 遍历指定页
  {
    for (i = X; i < X + Width; i++) // 遍历指定列
    {
      ...
      {
        OLED_DisplayBuf[j / 8][i] &=
            ~(0x01 << (j % 8)); // 将显存数组指定数据清零
      }
    }
  }
}
 ...
 * 参    数：String
 * 指定要显示的字符串，范围：ASCII码可见字符或中文字符组成的字符串 参
 * 数：FontSize 指定字体大小 范围：OLED_8X16		宽8像素，高16像素
 *                 OLED_6X8		宽6像素，高8像素
 * 返 回 值：无
...
void OLED_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize) {
  ...
  while (String[i] != '\0') // 遍历字符串
  {
    ...
    if ((String[i] & 0x80) == 0x00) // 第一个字节为0xxxxxxx
    {
      CharLength = 1; // 字符为1字节
      SingleChar[0] = String
          [i++]; // 将第一个字节写入SingleChar第0个位置，随后i指向下一个字节
    ...
    } else if ((String[i] & 0xE0) == 0xC0) // 第一个字节为110xxxxx
    {
      ...
      SingleChar[1] = String
          [i++]; // 将第二个字节写入SingleChar第1个位置，随后i指向下一个字节
 ...
 * 参    数：X
 * 指定格式化字符串左上角的横坐标，范围：-32768~32767，屏幕区域：0~127 参 数：Y
 * 指定格式化字符串左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63 参
 * 数：FontSize 指定字体大小 范围：OLED_8X16		宽8像素，高16像素
 *                 OLED_6X8		宽6像素，高8像素
 * 参    数：format
 * 指定要显示的格式化字符串，范围：ASCII码可见字符或中文字符组成的字符串 参
 * 数：... 格式化字符串参数列表 返 回 值：无 说
