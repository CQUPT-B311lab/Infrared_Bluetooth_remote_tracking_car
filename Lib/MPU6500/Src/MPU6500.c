...
void MPU6500_Calibrate_Gyro(uint16_t samples) {
  ...
  // (float)gyro_offset[0]*INV_GYRO_SCALE); printf("Y轴: %d LSB (%.2f°/s)\r\n",
  // gyro_offset[1], (float)gyro_offset[1]*INV_GYRO_SCALE); printf("Z轴: %d LSB
...
int MPU6500_Is_Device_Stationary(void) {
  ...
  const float ACCEL_VAR_THRESH = 0.001f; // 0.001 g²
  const float GYRO_VAR_THRESH = 0.1f;    // 0.1 (°/s)²
